...
public ResponseEntity<List<DishDto>> searchDishByDishType(@PathVariable("dishType")String dishType)throws DishException
{
	...
	public ResponseEntity<Set<DishDto>> searchDishByzip(@PathVariable("zip")int zip)throws RestaurantException
	{
		return new ResponseEntity<Set<DishDto>>(dishService.getDishByZip(zip),HttpStatus.FOUND);
	}
